...
// lib/robinhood-chain.ts's relayClaim), so Collect only needs a typed
// address, not a connected wallet. Stake is a pure bookkeeping action (see
// lib/pool.ts) -- no wallet step at all. Both are opt-in; nothing here is
...
export function ContestPrizeClaimPanel({
  ...
  useEffect(() => {
    Promise.all([
      fetch(`/api/contest-entries/${contestEntryId}/claim`).then((r) =>
        r.json(),
      ),
  ...
  async function submitStake() {
    ...
    try {
      const res = await fetch(
        `/api/contest-entries/${contestEntryId}/claim/stake`,
        { method: "POST" },
      );
      const data = await res.json();
